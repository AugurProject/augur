...
describe("Markets List", () => {
  ...
  describe("Market Cards", () => {
    ...
    it("should display category and tags in the top left corner of each card", async () => {
      await expect(yesNoMarket).toMatchElement("[data-testid='Category-0'", { text: 'AGRICULTURE', timeout: TIMEOUT })
    ...
    it("should display categorical market outcomes correctly", async () => {
      ...
      const categoricalMarket = await expect(page).toMatchElement("article", { text: "What will be the number one killer in the United States by January 1, 2019?", timeout: TIMEOUT })

      // display the top 3 outcomes for a Categorical Market, with a "+ N More" where N is the number of remaining outcomes
      await expect(categoricalMarket).toMatchElement(".market-outcomes-categorical-styles_MarketOutcomesCategorical__show-more", { text: "+ 3 more", timeout: TIMEOUT })
      ...
      await expect(categoricalMarket).toMatchElement(".market-outcomes-categorical-styles_MarketOutcomesCategorical__show-more", { text: "- 3 less", timeout: TIMEOUT })
    });
    ...
    it("should display a scale with the current mid-price for the market", async () => {
      ...
      await page.waitForSelector("[data-testid='markets-" + newMarket.id + "']", { timeout: TIMEOUT });
