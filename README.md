...
- If `Mainnet` isn't in the `Select Configuration` dropdown, this is because the installer doesn't overwrite the config.json file. If an alpha release was previously installed, `Mainnet` might not be in the config.json file. Delete the config.json file and restart the Augur application. 
