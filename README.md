...
- `Mainnet` isn't in the `Select Configuration` dropdown. The installer doesn't overwrite the config.json file. If an alpha release was previously installed mainnet might have not been in the config.json file. Delete the config.json file and restart the Augur application. 
