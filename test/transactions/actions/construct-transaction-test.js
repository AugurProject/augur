  ...
  describe('constructCollectedFeesTransaction', () => {
    ...
    test({
      ...
      assertions: () => {
        ...
        const repGain = createBigNumber(log.repGain)
        const initialRepBalance = createBigNumber(log.newRepBalance).minus(repGain).toFixed()
    ...
    test({
      ...
      assertions: () => {
        ...
        const totalReportingRep = createBigNumber(log.totalReportingRep)
        const percentRep = formatPercent(createBigNumber(log.initialRepBalance).dividedBy(totalReportingRep).times(100), { decimals: 0 })
      ...
      assertions: () => {
        ...
        const expected = {
          data: {
            balances: [
              {
                change: formatRep(createBigNumber(log._value, 10), { positiveSign: true }),
              },
            ],
          },
          type: 'Receive Tokens',
