...
describe('modules/bids-asks/actions/update-order-status.js', () => {
  ...
  describe('updateOrderStatus', () => {
    it(`shouldn't dispatch if order cannot be found`, () => {
      store.dispatch(updateOrderModule.updateOrderStatus('nonExistingOrderID', CLOSE_DIALOG_CLOSING, 'marketID', BUY));
      assert.lengthOf(store.getActions(), 0);

      store.dispatch(updateOrderModule.updateOrderStatus('orderID', CLOSE_DIALOG_CLOSING, 'nonExistingMarketID', BUY));
      assert.lengthOf(store.getActions(), 0);
    });

    it(`should dispatch action`, () => {
      store.dispatch(updateOrderModule.updateOrderStatus('0xdbd851cc394595f9c50f32c1554059ec343471b49f84a4b72c44589a25f70ff3', CLOSE_DIALOG_CLOSING, 'testMarketID', BUY));
