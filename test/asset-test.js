...
augur.web.account = {handle: handle, privateKey: privateKey, address: address};
...
test("AssetActions.updateAssets", function (t) {
    ...
    flux.register.UPDATE_PERCENT_LOADED_SUCCESS = function (payload) {
        ...
        t.equal(account, augur.web.account.address, "ConfigStore account == augur.web.account.address");
        t.true(abi.bignum(branch).eq(abi.bignum(augur.branches.dev)), "BranchStore currentBranch.id == augur.branches.dev");
