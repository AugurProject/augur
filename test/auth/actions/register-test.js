...
describe(`modules/auth/actions/register.js`, () => {
  ...
  sinon.stub(loadAccountDataStub, 'loadAccountData').callsFake(account => ({ type: loadAccountDataTestString }))

  const updateIsLoggedStub = {
    updateIsLogged: () => {}
  }
  sinon.stub(updateIsLoggedStub, 'updateIsLogged').callsFake(() => ({ type: 'update-is-logged' }))
