...
then
  echo -e "Your system already has access to Docker, you're good to go!\n";
else
  echo -e "You need Docker installed and configured in order to run the augur services. See: https://docs.docker.com/get-docker/ for instructions.\n";
  exit 1;
fi
...
if [ $? != 0 ]
then
  echo -e "Docker daemon not running. Cannot install.\n";
  exit 1;
else
...
print_menu() {
	echo -e "~~~~~~~~~~~~~~~~~~~~~"
	echo -e " Select Augur Environment"
	echo -e "~~~~~~~~~~~~~~~~~~~~~"
	echo -e "1. mainnet"
	echo -e "2. v2 (kovan)"
	echo -e "3. local"
...
read_options(){
	...
	case $choice in
		...
		2) echo -e "v2" ;;
		3) echo -e "local" ;;
		*) echo -e -e "${RED}Error...${STD}" && sleep 2
	esac
}
