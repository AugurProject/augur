...
export async function start() {
  ...
  const blockAndLogStreamerListener = BlockAndLogStreamerListener.create(provider, augur.events.parseLogs);

  const pouchDBFactory = PouchDBFactory({});
  const controller = new Controller<ethers.utils.BigNumber>(
      augur,
      TEST_NETWORK_ID,
