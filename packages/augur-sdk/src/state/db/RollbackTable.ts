...
export class RollbackTable extends AbstractTable {
  ...
  async standardRollback(blockNumber: number): Promise<void> {
    const docsToRemove = await this.table
      .where('blockNumber')
      .aboveOrEqual(blockNumber);
  ...
  async rollupRollback(blockNumber: number): Promise<void> {
    const docsForReplacement = await this.rollbackTable
      .where('[tableName+rollbackBlockNumber]')
      .between(
        [this.dbName, blockNumber],
        [this.dbName, Dexie.maxKey],
        true,
        true
      )
      .toArray();
  ...
  async prune(blockNumber: number) {
    ...
    await this.rollbackTable
      .where('[tableName+rollbackBlockNumber]')
      .between(
        [this.dbName, Dexie.minKey],
        [this.dbName, blockNumber - NUMBER_OF_BLOCKS_TO_KEEP]
      )
      .delete();
  }
}
