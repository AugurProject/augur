...
describe('BulkSyncStrategy', () => {
  ...
  test('should call onLogsAdded for each chunk with no overlap', async () => {
    ...
    expect(mockGetLogs.mock.calls[0][0]).toMatchObject({
      fromBlock: 10,
      toBlock: 14,
    });
    expect(mockGetLogs.mock.calls[1][0]).toMatchObject({
      fromBlock: 15,
      toBlock: 19,
    });
    expect(mockGetLogs.mock.calls[2][0]).toMatchObject({
      fromBlock: 20,
      toBlock: 24,
    });
    expect(mockGetLogs.mock.calls[3][0]).toMatchObject({
      fromBlock: 25,
      toBlock: 29,
    });
    expect(mockGetLogs.mock.calls[4][0]).toMatchObject({
      fromBlock: 30,
      toBlock: 30,
    });
