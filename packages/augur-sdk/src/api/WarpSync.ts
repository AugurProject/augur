...
export class WarpSync {
  ...
  async getWarpSyncHashFromMarket(
    ...
    const winningParticipant = this.augur.contracts.getReportingParticipant(
      winningBondAddress
    );
  ...
  async getLastWarpSyncData(universe: string): Promise<WarpSyncData> {
    const warpSync = this.augur.contracts.warpSync;
    const [warpSyncHash, timestamp] = ((await warpSync.data_(
      universe
    )) as unknown) as [BigNumber, BigNumber];
