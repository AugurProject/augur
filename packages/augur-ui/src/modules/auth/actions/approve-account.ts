...
import { TEN_TO_THE_EIGHTEENTH_POWER, ZERO } from "modules/common/constants";
...
export function checkAccountAllowance(callback: NodeStyleCallback = logError): ThunkAction<any, any, any, any> {
  return async (dispatch: ThunkDispatch<void, any, Action>, getState: () => AppState) => {
    ...
    if (loginAccount.allowance && loginAccount.allowance.gte(ZERO)) {
      callback(null, loginAccount.allowance);
    } else {
      const allowance = await getAllowance(loginAccount.address);
      callback(null, allowance);
      dispatch(updateLoginAccount({ allowance, allowanceFormatted: formatDai(allowance) }));
    }
  };
}
