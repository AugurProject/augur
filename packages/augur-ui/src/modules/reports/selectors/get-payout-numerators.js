...
export const getPayoutNumerators = (market, selectedOutcome, invalid) => {
  ...
  if (invalid) {
    const equalValue = createBigNumber(numTicks)
      .dividedBy(market.numOutcomes)
      .dp(0, BigNumber.ROUND_DOWN);
    return Array(market.numOutcomes).fill(equalValue);
  } else if (isScalar) {
    // selectedOutcome must be a BN as string
    const priceRange = createBigNumber(maxPrice).minus(
      createBigNumber(minPrice)
    );
    const reportNormalizedToZero = createBigNumber(selectedOutcome).minus(
      createBigNumber(minPrice)
    );
