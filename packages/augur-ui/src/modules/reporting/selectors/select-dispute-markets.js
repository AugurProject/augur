...
export const selectMarketsInDispute = createSelector(
  ...
  (markets, disputeOutcomes, universe) => {
    ...
    let filteredMarkets = markets.filter(market => market.reportingState === constants.REPORTING_STATE.AWAITING_FORK_MIGRATION || market.reportingState === constants.REPORTING_STATE.CROWDSOURCING_DISPUTE || market.id === universe.forkingMarket)
    ...
    Object.keys(filteredMarkets).forEach((marketKey) => {
      if (filteredMarkets[marketKey].disputeInfo) {
        filteredMarkets[marketKey].disputeInfo.highestPercentStaked = createBigNumber(0)
        Object.keys(filteredMarkets[marketKey].disputeInfo.stakes).forEach((stakeKey) => {
          ...
          filteredMarkets[marketKey].disputeInfo.highestPercentStaked = filteredMarkets[marketKey].disputeInfo.highestPercentStaked.toString()
        })
      }
    })
    filteredMarkets = orderBy(filteredMarkets, ['disputeInfo.disputeRound', 'disputeInfo.highestPercentStaked'], ['desc', 'desc'])
