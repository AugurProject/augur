...
export const filterBySearch = (search, keys) => items => {
  ...
  const checkArrayMatch = (item, keys, search) => {
    ...
    if (!parentValue && keys.length === 1)
      parentValue = getValue(item, keys[0]);
    ...
    if (Array.isArray(parentValue) && parentValue.length && keys.length !== 1) {
      return parentValue.some(
        value =>
          (value[keys[keys.length - 1]] || "").toLowerCase().indexOf(term) !==
          -1
      );
    } else if (
    ...
    const matchedSearch = searchArray.some(search =>
      keys.some(key => {
        ...
        if (typeof key === "string")
          return checkStringMatch(item[key] || "", term);
