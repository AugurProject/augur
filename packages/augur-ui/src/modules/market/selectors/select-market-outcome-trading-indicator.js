import { BUY_UP, BUY_DOWN, SELL_UP, SELL_DOWN, NONE, BUY } from 'modules/trade/constants/types'
import { createBigNumber } from 'utils/create-big-number'
import { orderBy, first } from 'lodash'

export const selectMarketOutcomeTradingIndicator = (marketTradingHistory, outcome) => {
...
function getTradeStatus(marketTradingHistory, marketId, outcomeId) {
  ...
  const trades = marketHistory.filter(t => t.outcome === parseInt(outcomeId, 10))
  if (!trades || trades.length === 0) return NONE
  const sortedTrades = orderBy(trades, ['timestamp', 'price'], ['desc', 'desc'])
  ...
  const isUp = createBigNumber(firstTrade.price).gt(createBigNumber(secondTrade.price))
  if (isUp) {
