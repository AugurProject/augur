...
import { placeBet, checkForDisablingPlaceBets } from 'utils/betslip-helpers';
...
export function BetslipReducer(state, action) {
  ...
  switch (action.type) {
    ...
    case MODIFY_BET: {
      ...
      if (betslipItems[marketId]?.orders) betslipItems[marketId].orders[orderId] = { ...order, shares, toWin };
