...
def test_log_requires(augur, universe):
    with raises(TransactionFailed):
        augur.logInitialReportSubmitted(universe.address, universe.address, universe.address, universe.address, 1, False, [0, 0, 100], "")

    with raises(TransactionFailed):
        augur.logInitialReportSubmitted(augur.address, augur.address, augur.address, augur.address, 1, False, [0, 0, 100], "")
