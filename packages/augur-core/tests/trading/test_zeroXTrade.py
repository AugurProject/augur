...
def test_basic_trading(contractsFixture, cash, market, universe):
    ...
    with TokenDelta(cash, -fix(10, 60), contractsFixture.accounts[0], "Tester 0 cash not taken"):
        with TokenDelta(cash, -fix(10, 40), contractsFixture.accounts[1], "Tester 1 cash not taken"):
            with PrintGasUsed(contractsFixture, "ZeroXTrade.trade", 0):
                amountRemaining = ZeroXTrade.trade(fillAmount, fingerprint, tradeGroupId, 0, orders, signatures, sender=contractsFixture.accounts[1], value=150000)
    ...
    amountRemaining = ZeroXTrade.trade(fillAmount + 10**17, fingerprint, tradeGroupId, 0, orders, signatures, sender=contractsFixture.accounts[2], value=150000)
    ...
    with raises(TransactionFailed):
        ZeroXTrade.trade(fillAmount, fingerprint, tradeGroupId, 0, orders, signatures, sender=contractsFixture.accounts[1], value=150000)
        ...
        with AssertLog(contractsFixture, "OrderEvent", orderEventLog):
            with TokenDelta(cash, -fix(2, 60), sender, "Creator cash not taken"):
                with TokenDelta(cash, -fix(2, 40), contractsFixture.accounts[2], "Taker cash not taken"):
                    assert ZeroXTrade.trade(fix(2), longTo32Bytes(11), tradeGroupID, 0, orders, signatures, sender=contractsFixture.accounts[2], value=150000) == 0
    ...
    with AssertLog(contractsFixture, "OrderEvent", orderEventLog):
        with TokenDelta(cash, -fix(1, 60), contractsFixture.accounts[1], "Creator cash not taken"):
            with TokenDelta(cash, -fix(1, 40), contractsFixture.accounts[2], "Taker cash not taken"):
                assert ZeroXTrade.trade(fix(1), longTo32Bytes(11), tradeGroupID, 0, orders, signatures, sender=contractsFixture.accounts[2], value=150000) == 0
    ...
    with TokenDelta(cash, -fix(4, 60), contractsFixture.accounts[1], "Creator cash not taken"):
        with TokenDelta(cash, -fix(1, 60), contractsFixture.accounts[3], "Creator cash not taken"):
            with TokenDelta(cash, -fix(5, 40), contractsFixture.accounts[2], "Taker cash not taken"):
                assert ZeroXTrade.trade(fix(5), longTo32Bytes(11), tradeGroupID, 0, orders, signatures, sender=contractsFixture.accounts[2], value=300000) == 0
    ...
    with TokenDelta(cash, -fix(1, 60), contractsFixture.accounts[1], "Creator cash not taken"):
        with TokenDelta(cash, -fix(2, 60), contractsFixture.accounts[3], "Creator cash not taken"):
            with TokenDelta(cash, -fix(3, 40), contractsFixture.accounts[2], "Taker cash not taken"):
                assert ZeroXTrade.trade(fix(3), longTo32Bytes(11), tradeGroupID, 0, orders, signatures, sender=contractsFixture.accounts[2], value=300000) == 0
    ...
    if finalized:
        if invalid:
            ...
            with TokenDelta(cash, totalFees, nextDisputeWindowAddress, "Dispute Window did not recieve the correct fees"):
                assert ZeroXTrade.trade(fix(1), fingerprint, tradeGroupID, 0, orders, signatures, sender=contractsFixture.accounts[2], value=150000) == 0
        else:
...
def test_order_creator_lacks_funds(contractsFixture, cash, market, universe):
    ...
    with TokenDelta(cash, 0, contractsFixture.accounts[1], "Creator cash not received"):
        with TokenDelta(cash, 0, contractsFixture.accounts[2], "Taker cash not taken"):
            assert ZeroXTrade.trade(fix(1), longTo32Bytes(11), tradeGroupID, 0, orders, signatures, sender=contractsFixture.accounts[2], value=150000) == fix(1)
...
def test_fill_nothing_failure(contractsFixture, cash, market, universe):
    ...
    with raises(TransactionFailed):
        ZeroXTrade.trade(fix(1), longTo32Bytes(11), tradeGroupID, 0, [], [], sender=contractsFixture.accounts[2], value=150000) == fix(1)
    ...
    with TokenDelta(cash, -fix(1, 60), gnosisSafeAddress, "Tester 0 cash not taken"):
        with TokenDelta(cash, -fix(1, 40), gnosisSafeAddress2, "Tester 1 cash not taken"):
            with PrintGasUsed(contractsFixture, "ZeroXTrade.trade", 0):
                ...
                tradeData = ZeroXTrade.trade_encode(fillAmount, fingerprint, tradeGroupId, 0, orders, signatures)
...
def test_protocol_fee_coverage(contractsFixture, cash, market):
    ...
    with raises(TransactionFailed):
        assert ZeroXTrade.trade(fix(5), longTo32Bytes(11), tradeGroupID, 0, orders, signatures, sender=account) == 0
    ...
    assert ZeroXTrade.trade(fix(5), longTo32Bytes(11), tradeGroupID, cost, orders, signatures, sender=account) == 0
