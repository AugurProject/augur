...
def test_publicCreateOrder_bid(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix(1, 4000), tester.k0, "create order"):
        orderID = createOrder.publicCreateOrder(BID, fix(1), 4000, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), "7")
        assert orderID
...
def test_publicCreateOrder_ask(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix(1, 6000), tester.k0, "create order"):
        orderID = createOrder.publicCreateOrder(ASK, fix(1), 4000, market.address, 0, longTo32Bytes(0), longTo32Bytes(0), "7")
...
def test_publicCreateOrder_List_Logic(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix(1, 4010), tester.k0, "create order 1"):
        orderID_10 = createOrder.publicCreateOrder(BID, fix(1), 4010, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), "1")
    with BuyWithCash(cash, fix(1, 4008), tester.k0, "create order 2"):
        orderID_8 = createOrder.publicCreateOrder(BID, fix(1), 4008, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), "2")
    with BuyWithCash(cash, fix(1, 4006), tester.k0, "create order 3"):
        orderID_6 = createOrder.publicCreateOrder(BID, fix(1), 4006, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), "3")
    with BuyWithCash(cash, fix(1, 4002), tester.k0, "create order 4"):
        orderID_2 = createOrder.publicCreateOrder(BID, fix(1), 4002, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), "4")
    with BuyWithCash(cash, fix(1, 4001), tester.k0, "create order 5"):
        orderID_1 = createOrder.publicCreateOrder(BID, fix(1), 4001, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), "5")
    with BuyWithCash(cash, fix(1, 4007), tester.k0, "create order 6"):
        orderID_7 = createOrder.publicCreateOrder(BID, fix(1), 4007, market.address, 1, orderID_10, orderID_1, "6")
    assert orderID_7
    ...
    with BuyWithCash(cash, fix(1, 4005), tester.k0, "create order 7"):
        orderID_5 = createOrder.publicCreateOrder(BID, fix(1), 4005, market.address, 1, orderID_6, orderID_1, "7")
    assert orderID_5
    ...
    with BuyWithCash(cash, fix(1, 4003), tester.k0, "create order 8"):
        orderID_3 = createOrder.publicCreateOrder(BID, fix(1), 4003, market.address, 1, orderID_5, orderID_2, "8")
    assert orderID_3
...
def test_publicCreateOrder_bid2(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('1', '4000'), tester.k1, "create order"):
        with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
            orderID = createOrder.publicCreateOrder(orderType, amount, fxpPrice, market.address, outcome, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender=tester.k1)
...
def test_ask_withPartialShares(contractsFixture, universe, cash, market):
    ...
    with BuyWithCash(cash, fix('6000'), tester.k1, "buy complete set"):
        with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
            orderID = createOrder.publicCreateOrder(ASK, fix(3), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), "42", sender=tester.k1)
    assert cash.balanceOf(tester.a1) == fix('0')
...
def test_ask_withSharesIgnored(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('5000'), tester.k1, "create order"):
        with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
            orderID = createOrder.publicCreateOrder(BID, fix(1), 5000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), "42", True, sender=tester.k1)
    assert cash.balanceOf(tester.a1) == fix('0')
    assert yesShareToken.balanceOf(tester.a1) == fix(2)
    assert noShareToken.balanceOf(tester.a1) == fix(2)
...
def test_publicCreateOrders(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, value, tester.k0, "create order"):
        orderIDs = createOrder.publicCreateOrders(outcomes, types, attoshareAmounts, prices, market.address, False, "42")
        assert orderIDs
