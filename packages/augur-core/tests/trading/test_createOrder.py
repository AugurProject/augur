...
from utils import longTo32Bytes, bytesToHexString, fix, AssertLog, BuyWithCash, stringToBytes
...
def test_publicCreateOrder_0_shares(contractsFixture, cash, market):
    ...
    with raises(TransactionFailed):
        createOrder.publicCreateOrder(BID, 0, 4000, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(7))
...
def test_publicCreateOrder_bid(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix(1, 4000), tester.k0, "create order"):
        orderID = createOrder.publicCreateOrder(BID, fix(1), 4000, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(7), False)
        assert orderID
...
def test_publicCreateOrder_ask(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix(1, 6000), tester.k0, "create order"):
        orderID = createOrder.publicCreateOrder(ASK, fix(1), 4000, market.address, 0, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(7), False)
...
def test_publicCreateOrder_List_Logic(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix(1, 4010), tester.k0, "create order 1"):
        orderID_10 = createOrder.publicCreateOrder(BID, fix(1), 4010, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(1), False)
    with BuyWithCash(cash, fix(1, 4008), tester.k0, "create order 2"):
        orderID_8 = createOrder.publicCreateOrder(BID, fix(1), 4008, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(2), False)
    with BuyWithCash(cash, fix(1, 4006), tester.k0, "create order 3"):
        orderID_6 = createOrder.publicCreateOrder(BID, fix(1), 4006, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(3), False)
    with BuyWithCash(cash, fix(1, 4002), tester.k0, "create order 4"):
        orderID_2 = createOrder.publicCreateOrder(BID, fix(1), 4002, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(4), False)
    with BuyWithCash(cash, fix(1, 4001), tester.k0, "create order 5"):
        orderID_1 = createOrder.publicCreateOrder(BID, fix(1), 4001, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(5), False)
    with BuyWithCash(cash, fix(1, 4007), tester.k0, "create order 6"):
        orderID_7 = createOrder.publicCreateOrder(BID, fix(1), 4007, market.address, 1, orderID_10, orderID_1, longTo32Bytes(6), False)
    assert orderID_7
    ...
    with BuyWithCash(cash, fix(1, 4005), tester.k0, "create order 7"):
        orderID_5 = createOrder.publicCreateOrder(BID, fix(1), 4005, market.address, 1, orderID_6, orderID_1, longTo32Bytes(7), False)
    assert orderID_5
    ...
    with BuyWithCash(cash, fix(1, 4003), tester.k0, "create order 8"):
        orderID_3 = createOrder.publicCreateOrder(BID, fix(1), 4003, market.address, 1, orderID_5, orderID_2, longTo32Bytes(8), False)
    assert orderID_3
...
def test_publicCreateOrder_bid2(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('1', '4000'), tester.k1, "create order"):
        with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
            orderID = createOrder.publicCreateOrder(orderType, amount, fxpPrice, market.address, outcome, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, sender=tester.k1)
...
def test_createOrder_failure(contractsFixture, cash, market):
    ...
    with raises(TransactionFailed):
        createOrder.createOrder(tester.a1, ASK, fix(1), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)

    # createOrder exceptions (pre-escrowFunds)
    with raises(TransactionFailed):
        createOrder.publicCreateOrder(3, fix(1), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)

    # escrowFundsForBid exceptions
    with raises(TransactionFailed):
        createOrder.publicCreateOrder(BID, fix(1), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)
    ...
    with raises(TransactionFailed):
        createOrder.publicCreateOrder(ASK, 1, 1, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)

    with raises(TransactionFailed):
        createOrder.publicCreateOrder(ASK, fix(1), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)
    ...
    with raises(TransactionFailed):
        createOrder.publicCreateOrder(ASK, fix(1), 12000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)

    with raises(TransactionFailed):
        createOrder.publicCreateOrder(ASK, fix(1), 4000, tester.a1, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)

    assert createOrder.publicCreateOrder(ASK, fix(1), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1) != 0, "Order ID should be non-zero"
...
def test_ask_withPartialShares(contractsFixture, universe, cash, market):
    ...
    with BuyWithCash(cash, fix('6000'), tester.k1, "buy complete set"):
        with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
            orderID = createOrder.publicCreateOrder(ASK, fix(3), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, sender=tester.k1)
    assert cash.balanceOf(tester.a1) == fix('0')
...
def test_duplicate_creation_transaction(contractsFixture, cash, market):
    ...
    # FFDFSA
    with raises(TransactionFailed):
        createOrder.publicCreateOrder(BID, fix(1), 4000, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(7), False, value = fix(1, 4000))
...
def test_ask_withSharesIgnored(contractsFixture, cash, market):
    ...
    with raises(TransactionFailed):
        createOrder.publicCreateOrder(BID, fix(1), 5000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), True, sender=tester.k1)
    ...
    with BuyWithCash(cash, fix('5000'), tester.k1, "create order"):
        with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
            orderID = createOrder.publicCreateOrder(BID, fix(1), 5000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), True, sender=tester.k1)
    assert cash.balanceOf(tester.a1) == fix('0')
    assert yesShareToken.balanceOf(tester.a1) == fix(2)
    assert noShareToken.balanceOf(tester.a1) == fix(2)
...
def test_publicCreateOrders(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, value, tester.k0, "create order"):
        orderIDs = createOrder.publicCreateOrders(outcomes, types, attoshareAmounts, prices, market.address, False, longTo32Bytes(42))
        assert orderIDs
