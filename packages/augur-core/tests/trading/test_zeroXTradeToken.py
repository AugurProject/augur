...
def test_playground(contractsFixture, cash, market, universe):
    ...
    rawZeroXOrderData, orderHash = zeroXTradeToken.createZeroXOrder(BID, fix(2), 60, market.address, YES, False, nullAddress, expirationTime, salt)
            ...
            with TokenDelta(cash, -fix(1, 60), contractsFixture.accounts[0], "Tester 0 cash not taken"):
                with TokenDelta(cash, -fix(1, 40), contractsFixture.accounts[1], "Tester 1 cash not taken"):
                    with PrintGasUsed(contractsFixture, "zeroXTradeToken.trade", 0):
                        amountRemaining = zeroXTradeToken.trade(fillAmount, ignoreShares, affiliateAddress, tradeGroupId, orders, signatures, sender=contractsFixture.accounts[1])
    ...
    amountRemaining = zeroXTradeToken.trade(fillAmount + 1, ignoreShares, affiliateAddress, tradeGroupId, orders, signatures, sender=contractsFixture.accounts[2])
    assert amountRemaining == 1

    # The order is completely filled so further attempts to take it will fail
    assert cash.faucet(fix(1, 60))
    assert cash.faucet(fix(1, 40), sender=contractsFixture.accounts[1])
    with raises(TransactionFailed):
        zeroXTradeToken.trade(fillAmount, ignoreShares, affiliateAddress, tradeGroupId, orders, signatures, sender=contractsFixture.accounts[1])
