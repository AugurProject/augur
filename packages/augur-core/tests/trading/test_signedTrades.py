...
def test_signed_trade(contractsFixture, universe, market, cash, augur):
    ...
    tradeHash = trade.getTradeHash(BID, market.address, YES, fix(10), 1000, tester.a0, False, expirationTimestampInSec, 42, payment)
    v, r, s = createTrade(tradeHash)

    # Fail with no Cash deposited
    with raises(TransactionFailed):
        trade.executeSignedTrade(
            BID, market.address, YES, fix(10), 1000, tester.a0, False, expirationTimestampInSec, 42, payment,
            v,
            r,
            s,
            sender=tester.k1)

    assert cash.faucet(fix(10000) + 100)
    assert cash.approve(augur.address, fix(10000) + 100)
    ...
    assert orders.getOrderMoneyEscrowed(orderID) == fix(10000)
...
def test_signed_fill(contractsFixture, universe, market, cash, augur):
    ...
    with BuyWithCash(cash, fix(90000), tester.k1, "place an order"):
        orderID = createOrder.publicCreateOrder(ASK, fix(10), 1000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, nullAddress, sender = tester.k1)
    ...
    assert yesShareToken.balanceOf(tester.a0) == fix(10)
