...
def test_one_bid_on_books_buy_partial_order(contractsFixture, cash, market):
    ...
    # create order
    with BuyWithCash(cash, fix('2', '60'), tester.k1, "create order"):
...
def test_one_bid_on_books_buy_partial_order_fill_loop_limit(contractsFixture, cash, market, universe):
    ...
    # create order
    with BuyWithCash(cash, fix('2', '60'), tester.k1, "trade 1"):
        orderID = createOrder.publicCreateOrder(BID, fix(2), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
...
def test_one_bid_on_books_buy_excess_order(withTotalCost, contractsFixture, cash, market, universe):
    ...
    # create order
    with BuyWithCash(cash, fix('4', '60'), tester.k1, "create order"):
    ...
    with AssertLog(contractsFixture, "OrderEvent", orderFilledEventLog):
        with AssertLog(contractsFixture, "OrderEvent", orderCreatedEventLog, skip=1):
            if withTotalCost:
                ...
                with BuyWithCash(cash, fix('5', '40'), tester.k2, "trade"):
                    fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(5), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender=tester.k2)
...
def test_two_bids_on_books_buy_both(contractsFixture, cash, market):
    ...
    # create order 1
    with BuyWithCash(cash, fix('4', '60'), tester.k1, "create order"):
        orderID1 = createOrder.publicCreateOrder(BID, fix(4), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('1', '60'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(BID, fix(1), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k3)

    # fill best order
    with PrintGasUsed(contractsFixture, "Fill two", 0):
        with BuyWithCash(cash, fix('5', '40'), tester.k2, "fill best orders"):
            fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(5), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k2)
...
def test_two_bids_on_books_buy_one_with_limit(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('4', '60'), tester.k1, "create order 1"):
        orderID1 = createOrder.publicCreateOrder(BID, fix(4), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    with BuyWithCash(cash, fix('1', '60'), tester.k3, "create order 2"):
        orderID2 = createOrder.publicCreateOrder(BID, fix(1), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k3)

    # fill best order
    with PrintGasUsed(contractsFixture, "Fill two", 0):
        with BuyWithCash(cash, fix('4', '40'), tester.k2, "buy complete set"):
            fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(5), 60, "0", "0", tradeGroupID, 1, False, nullAddress, nullAddress, sender = tester.k2)
...
def test_two_bids_on_books_buy_full_and_partial(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('12', '60'), tester.k1, "create order"):
        orderID1 = createOrder.publicCreateOrder(BID, fix(12), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('7', '60'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(BID, fix(7), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k3)

    # fill best order
    with BuyWithCash(cash, fix('15', '40'), tester.k2, "trade"):
        fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(15), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k2)
...
def test_two_bids_on_books_buy_one_full_then_create(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('12', '60'), tester.k1, "create order"):
        orderID1 = createOrder.publicCreateOrder(BID, fix(12), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('7', '50'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(BID, fix(7), 50, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k3)
...
def test_one_ask_on_books_buy_full_order(withTotalCost, contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('12', '40'), tester.k1, "buy complete set"):
        orderID = createOrder.publicCreateOrder(ASK, fix(12), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)

    # fill best order
    if withTotalCost:
        with BuyWithCash(cash, fix('12', '60'), tester.k2, "buy complete set"):
            fillOrderID = trade.publicTradeWithTotalCost(LONG, market.address, YES, fix(12, 60), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k2)
    else:
        with BuyWithCash(cash, fix('12', '60'), tester.k2, "buy complete set"):
            fillOrderID = trade.publicTrade(LONG, market.address, YES, fix(12), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k2)
...
def test_one_ask_on_books_buy_partial_order(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('7', '60'), tester.k2, "fill best order"):
        fillOrderID = trade.publicTrade(LONG, market.address, YES, fix(7), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k2)
...
def test_one_ask_on_books_buy_excess_order(contractsFixture, cash, market, universe):
    ...
    # fill best order
    with BuyWithCash(cash, fix('15', '60'), tester.k2, "buy complete set"):
        fillOrderID = trade.publicTrade(LONG,market.address, YES, fix(15), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k2)
...
def test_two_asks_on_books_buy_both(contractsFixture, cash, market, universe):
    ...
    # create order 1
    with BuyWithCash(cash, fix('12', '40'), tester.k1, "buy complete set"):
        orderID1 = createOrder.publicCreateOrder(ASK, fix(12), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('3', '40'), tester.k3, "buy complete set"):
        orderID2 = createOrder.publicCreateOrder(ASK, fix(3), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k3)
...
def test_two_asks_on_books_buy_full_and_partial(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('12', '40'), tester.k1, "buy complete set"):
        orderID1 = createOrder.publicCreateOrder(ASK, fix(12), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    # create order
    with BuyWithCash(cash, fix('7', '40'), tester.k3, "buy complete set"):
        orderID2 = createOrder.publicCreateOrder(ASK, fix(7), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k3)
...
def test_two_asks_on_books_buy_one_full_then_create(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('12', '40'), tester.k1, "create order"):
        orderID1 = createOrder.publicCreateOrder(ASK, fix(12), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('7', '30'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(ASK, fix(7), 70, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k3)

    # fill/create
    with BuyWithCash(cash, fix('15', '60'), tester.k2, "fill and create order"):
        fillOrderID = trade.publicTrade(LONG,market.address, YES, fix(15), 60, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k2)
...
def test_take_best_order(withTotalCost, contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('1', '40'), tester.k1, "create order"):
        orderID = createOrder.publicCreateOrder(ASK, fix(1), 60, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, nullAddress, sender=tester.k1)
    assert orderID

    # fill order with cash using on-chain matcher
    if withTotalCost:
        with BuyWithCash(cash, fix('1', '60'), tester.k2, "fill best order"):
            assert trade.publicFillBestOrderWithTotalCost(BID, market.address, YES, fix(1, 60), 60, "43", 6, False, nullAddress, nullAddress, sender=tester.k2) == 0
    else:
        with BuyWithCash(cash, fix('1', '60'), tester.k2, "fill best order"):
            assert trade.publicFillBestOrder(BID, market.address, YES, fix(1), 60, "43", 6, False, nullAddress, nullAddress, sender=tester.k2) == 0
...
def test_take_best_order_multiple_orders(contractsFixture, cash, market):
    ...
    for i in range(numOrders):
        with BuyWithCash(cash, fix('1', 40 - i), tester.k1, "create order"):
            orderID = createOrder.publicCreateOrder(ASK, fix(1), 60 + i, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, nullAddress, sender=tester.k1)
        assert orderID
...
def test_take_best_order_with_shares_escrowed_buy_with_shares_categorical(contractsFixture, cash, categoricalMarket, universe):
    ...
    # create order with shares
    orderID = createOrder.publicCreateOrder(ASK, fix(1), 60, market.address, 0, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(42), False, nullAddress, sender=tester.k1)
    assert orderID
    ...
    with TokenDelta(cash, expectedTester1Payout, tester.a1, "Tester 1 ETH delta wrong"):
        with PrintGasUsed(contractsFixture, "categoricalFill", 0):
            assert trade.publicFillBestOrder(BID, market.address, 0, fix(1), 60, "43", 6, False, nullAddress, nullAddress, sender=tester.k2) == 0
...
def test_trade_with_self_take_order_make_order(withTotalCost, contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, createCost, tester.k1, "create order"):
        orderID = createOrder.publicCreateOrder(ASK, fix('0.003'), 40, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)

    # fill best order
    takeCost = fix('1', '50')
    with BuyWithCash(cash, takeCost, tester.k1, "publicTradeWithTotalCost"):
        if withTotalCost:
            fillOrderID = trade.publicTradeWithTotalCost(BID, market.address, YES, takeCost, 50, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k1)
        else:
            fillOrderID = trade.publicTrade(BID, market.address, YES, fix(1), 50, "0", "0", tradeGroupID, 6, False, nullAddress, nullAddress, sender = tester.k1)
...
def test_create_order_after_exhausting_book(isMatch, contractsFixture, cash, market):
    ...
    createCost = fix('1', '60')
    with BuyWithCash(cash, createCost, tester.k1, "create order"):
        orderID = createOrder.publicCreateOrder(ASK, fix('1'), 40, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    if isMatch:
        createCost = fix('1', '50')
        with BuyWithCash(cash, createCost, tester.k1, "create matching order"):
            orderID2 = createOrder.publicCreateOrder(ASK, fix('1'), 50, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
    else:
        createCost = fix('1', '30')
        with BuyWithCash(cash, createCost, tester.k1, "create non-matching order"):
            orderID2 = createOrder.publicCreateOrder(ASK, fix('1'), 70, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, False, nullAddress, sender = tester.k1)
...
def test_take_best_order_with_shares_escrowed_buy_with_cash_by_ignoring_shares(useFill, contractsFixture, cash, market):
    ...
    with raises(TransactionFailed):
        if useFill:
            trade.publicFillBestOrder(BID, market.address, 0, fix(1), 60, longTo32Bytes(43), 6, True, nullAddress, nullAddress, sender=tester.k2) == 0
        else:
            trade.publicTrade(BID, market.address, 0, fix(1), 60, longTo32Bytes(0), longTo32Bytes(0), "43", 6, True, nullAddress, nullAddress, sender=tester.k2)

    # fill order with cash using on-chain matcher and ignoring owned shares
    if useFill:
        with BuyWithCash(cash, fix(60), tester.k2, "fill best order"):
            assert trade.publicFillBestOrder(BID, market.address, 0, fix(1), 60, longTo32Bytes(43), 6, True, nullAddress, nullAddress, sender=tester.k2) == 0
    else:
        with BuyWithCash(cash, fix(60), tester.k2, "trade"):
            assert trade.publicTrade(BID, market.address, 0, fix(1), 60, longTo32Bytes(0), longTo32Bytes(0), longTo32Bytes(43), 6, True, nullAddress, nullAddress, sender=tester.k2)
...
def test_fees_from_trades(finalized, contractsFixture, cash, market):
    ...
    expectedAffiliateFees = fix(100) * 0.01 * .25
