...
def test_one_bid_on_books_buy_full_order(withSelf, contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('2', '6000'), sender, "create order"):
        orderID = createOrder.publicCreateOrder(BID, fix(2), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = sender)
    ...
    with BuyWithCash(cash, fix('2', '4000'), tester.k2, "fill order"):
        with AssertLog(contractsFixture, "OrderFilled", orderFilledLog):
            assert trade.publicTrade(SHORT,market.address, YES, fix(2), 6000, "0", "0", tradeGroupID, 6, sender=tester.k2)
    ...
    with BuyWithCash(cash, fix('1', '4000'), tester.k2, "trade"):
        with AssertLog(contractsFixture, "OrderFilled", orderFilledLog):
            with PrintGasUsed(contractsFixture, "publicTrade", 0):
                fillOrderID = trade.publicTrade(1, market.address, YES, fix(1), 6000, "0", "0", tradeGroupID, 6, sender = tester.k2)
...
def test_one_bid_on_books_buy_excess_order(withTotalCost, contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('4', '6000'), tester.k1, "create order"):
        orderID = createOrder.publicCreateOrder(BID, fix(4), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)
        ...
        with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
            if withTotalCost:
                with BuyWithCash(cash, fix('5', '4000'), tester.k2, "tradeWithTotalCost"):
                    fillOrderID = trade.publicTradeWithTotalCost(SHORT,market.address, YES, fix(5, 6000), 6000, "0", "0", tradeGroupID, 6, sender=tester.k2)
            else:
                with BuyWithCash(cash, fix('5', '4000'), tester.k2, "trade"):
                    fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(5), 6000, "0", "0", tradeGroupID, 6, sender=tester.k2)
...
def test_two_bids_on_books_buy_both(contractsFixture, cash, market):
    ...
    # create order 1
    with BuyWithCash(cash, fix('4', '6000'), tester.k1, "create order"):
        orderID1 = createOrder.publicCreateOrder(BID, fix(4), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('1', '6000'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(BID, fix(1), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k3)
...
def test_two_bids_on_books_buy_one_with_limit(contractsFixture, cash, market, universe):
    ...
    with PrintGasUsed(contractsFixture, "Fill two", 0):
        with BuyWithCash(cash, fix('4', '4000'), tester.k2, "buy complete set"):
            fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(5), 6000, "0", "0", tradeGroupID, 1, sender = tester.k2)
            pass
...
def test_two_bids_on_books_buy_full_and_partial(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('12', '6000'), tester.k1, "create order"):
        orderID1 = createOrder.publicCreateOrder(BID, fix(12), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('7', '6000'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(BID, fix(7), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k3)

    # fill best order
    with BuyWithCash(cash, fix('15', '4000'), tester.k2, "trade"):
        fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(15), 6000, "0", "0", tradeGroupID, 6, sender = tester.k2)
...
def test_two_bids_on_books_buy_one_full_then_create(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('7', '5000'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(BID, fix(7), 5000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k3)
...
def test_one_ask_on_books_buy_full_order(withTotalCost, contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('12', '4000'), tester.k1, "buy complete set"):
        orderID = createOrder.publicCreateOrder(ASK, fix(12), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)

    # fill best order
    if withTotalCost:
        with BuyWithCash(cash, fix('12', '6000'), tester.k2, "buy complete set"):
            fillOrderID = trade.publicTradeWithTotalCost(LONG, market.address, YES, fix(12, 6000), 6000, "0", "0", tradeGroupID, 6, sender = tester.k2)
    else:
        with BuyWithCash(cash, fix('12', '6000'), tester.k2, "buy complete set"):
            fillOrderID = trade.publicTrade(LONG, market.address, YES, fix(12), 6000, "0", "0", tradeGroupID, 6, sender = tester.k2)
...
def test_one_ask_on_books_buy_partial_order(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('7', '6000'), tester.k2, "fill best order"):
        fillOrderID = trade.publicTrade(LONG, market.address, YES, fix(7), 6000, "0", "0", tradeGroupID, 6, sender = tester.k2)
...
def test_one_ask_on_books_buy_excess_order(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('15', '6000'), tester.k2, "buy complete set"):
        fillOrderID = trade.publicTrade(LONG,market.address, YES, fix(15), 6000, "0", "0", tradeGroupID, 6, sender = tester.k2)
...
def test_two_asks_on_books_buy_both(contractsFixture, cash, market, universe):
    ...
    # create order 1
    with BuyWithCash(cash, fix('12', '4000'), tester.k1, "buy complete set"):
        orderID1 = createOrder.publicCreateOrder(ASK, fix(12), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)
    # create order 2
    with BuyWithCash(cash, fix('3', '4000'), tester.k3, "buy complete set"):
        orderID2 = createOrder.publicCreateOrder(ASK, fix(3), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k3)
...
def test_two_asks_on_books_buy_full_and_partial(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('7', '4000'), tester.k3, "buy complete set"):
        orderID2 = createOrder.publicCreateOrder(ASK, fix(7), 6000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k3)
...
def test_two_asks_on_books_buy_one_full_then_create(contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, fix('7', '3000'), tester.k3, "create order"):
        orderID2 = createOrder.publicCreateOrder(ASK, fix(7), 7000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k3)
...
def test_take_best_order(withTotalCost, contractsFixture, cash, market):
    ...
    if withTotalCost:
        with BuyWithCash(cash, fix('1', '6000'), tester.k2, "fill best order"):
            assert trade.publicFillBestOrderWithTotalCost(BID, market.address, YES, fix(1, 6000), 6000, "43", 6, sender=tester.k2) == 0
    else:
        with BuyWithCash(cash, fix('1', '6000'), tester.k2, "fill best order"):
            assert trade.publicFillBestOrder(BID, market.address, YES, fix(1), 6000, "43", 6, sender=tester.k2) == 0
...
def test_take_best_order_multiple_orders(contractsFixture, cash, market):
    ...
    for i in range(numOrders):
        with BuyWithCash(cash, fix('1', 4000 - i), tester.k1, "create order"):
            orderID = createOrder.publicCreateOrder(ASK, fix(1), 6000 + i, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), "42", sender=tester.k1)
        assert orderID
    ...
    with PrintGasUsed(contractsFixture, "fill multiple asks", 0):
        ...
        assert trade.publicFillBestOrder(BID, market.address, YES, fix(numOrders), price, "43", 6, sender=tester.k1) == 0
...
def test_trade_with_self(contractsFixture, cash, market, universe):
    ...
    with BuyWithCash(cash, fix('5', '4000'), tester.k1, "trade"):
        with AssertLog(contractsFixture, "OrderFilled", orderFilledLog):
            with AssertLog(contractsFixture, "OrderCreated", orderCreatedLog):
                fillOrderID = trade.publicTrade(SHORT,market.address, YES, fix(5), 6000, "0", "0", tradeGroupID, 6, sender = tester.k1)
...
def test_trade_with_self_take_order_make_order(withTotalCost, contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, createCost, tester.k1, "create order"):
        orderID = createOrder.publicCreateOrder(ASK, fix('0.003'), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)

    # fill best order
    takeCost = fix('1', '5000')
    with BuyWithCash(cash, takeCost, tester.k1, "publicTradeWithTotalCost"):
        if withTotalCost:
            fillOrderID = trade.publicTradeWithTotalCost(BID, market.address, YES, takeCost, 5000, "0", "0", tradeGroupID, 6, sender = tester.k1)
        else:
            fillOrderID = trade.publicTrade(BID, market.address, YES, fix(1), 5000, "0", "0", tradeGroupID, 6, sender = tester.k1)
...
def test_create_order_after_exhausting_book(isMatch, contractsFixture, cash, market):
    ...
    with BuyWithCash(cash, createCost, tester.k1, "create order"):
        orderID = createOrder.publicCreateOrder(ASK, fix('1'), 4000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)
    if isMatch:
        createCost = fix('1', '5000')
        with BuyWithCash(cash, createCost, tester.k1, "create matching order"):
            orderID2 = createOrder.publicCreateOrder(ASK, fix('1'), 5000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)
    else:
        createCost = fix('1', '3000')
        with BuyWithCash(cash, createCost, tester.k1, "create non-matching order"):
            orderID2 = createOrder.publicCreateOrder(ASK, fix('1'), 7000, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender = tester.k1)

    # fill best order, isMatch determines if one of the orders
    takeCost = fix('2', '6000')
    with BuyWithCash(cash, takeCost, tester.k0, "trade"):
        fillOrderID = trade.publicTrade(BID, market.address, YES, fix(2), 6000, "0", "0", tradeGroupID, 6)
        if isMatch:
