...
def test_bootstrap(localFixture, universe, reputationToken, auction, time, cash):
    ...
    with BuyWithCash(cash, cost, localFixture.accounts[0], "trade cash for rep"):
        with TokenDelta(cash, cost, auction.address, "CASH was not transfered to auction correctly"):
            with TokenDelta(repAuctionToken, cost, localFixture.accounts[0], "REP auction token was not transferred to the user correctly"):
    ...
    with TokenDelta(reputationToken, expectedREP, localFixture.accounts[0], "REP was not distributed correctly from auction token redemption"):
        repAuctionToken.redeem()
    ...
    with TokenDelta(cashAuctionToken, cost, localFixture.accounts[0], "CASH auction token was not transferred to the user correctly"):
    ...
    with TokenDelta(cash, cash.balanceOf(cashAuctionToken.address), localFixture.accounts[0], "Cash redemption from cash auction token did not work correctly"):
...
def test_reporting_fee_from_auction(localFixture, universe, auction, reputationToken, time, cash):
    ...
    with BuyWithCash(cash, cost, localFixture.accounts[0], "trade cash for rep"):
        with TokenDelta(cash, cost, auction.address, "CASH was not transferred to auction correctly"):
            with TokenDelta(repAuctionToken, cost, localFixture.accounts[0], "REP was not transferred to the user correctly"):
                assert auction.tradeCashForRep(repAmount)
    ...
    lowerBoundRepPrice = int(Decimal(auction.initialAttoCashBalance()) * Decimal(10**18) / Decimal(cashAuctionToken.maxSupply()))
    upperBoundRepPrice = int(Decimal(repAuctionToken.maxSupply()) * Decimal(10**18) / Decimal(auction.initialAttoRepBalance()))
    ...
    assert auction.initialCashSalePrice() == int(4 * Decimal(10**36) / Decimal(derivedRepPrice))
    ...
    lowerBoundRepPrice = int(Decimal(auction.initialAttoCashBalance()) * Decimal(10**18) / Decimal(cashAuctionToken.maxSupply()))
    upperBoundRepPrice = int(Decimal(repAuctionToken.maxSupply()) * Decimal(10**18) / Decimal(auction.initialAttoRepBalance()))
    newDerivedRepPrice = int(Decimal(lowerBoundRepPrice + upperBoundRepPrice) / 2)
    ...
    assert auction.initialCashSalePrice() == int(4 * Decimal(10**36) / Decimal(newDerivedRepPrice))
...
def universe(localFixture, kitchenSinkSnapshot):
    return kitchenSinkSnapshot['universe']
