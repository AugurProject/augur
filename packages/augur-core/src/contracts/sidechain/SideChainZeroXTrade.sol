...
contract SideChainZeroXTrade is Initializable, IZeroXTrade, IERC1155 {
    ...
    ISideChainFillOrder public fillOrder;
    ICash public cash;
    ...
    function initialize(ISideChainAugur _augur, ISideChainAugurTrading _augurTrading) public beforeInitialized {
        ...
        fillOrder = ISideChainFillOrder(_augurTrading.lookup("FillOrder"));
        require(fillOrder != ISideChainFillOrder(0));
    ...
    function balanceOf(address owner, uint256 id) external view returns (uint256) {
        ...
        if (_orderType == Order.Types.Ask) {
            return askBalance(owner, _market, _outcome, _price);
        } else if (_orderType == Order.Types.Bid) {
            return bidBalance(owner, _market, _outcome, _price);
        }
    }
    ...
    function bidBalance(address _owner, address _market, uint8 _outcome, uint256 _price) public view returns (uint256) {
    ...
    function askBalance(address _owner, address _market, uint8 _outcome, uint256 _price) public view returns (uint256) {
        uint256 _attoSharesOwned = shareToken.balanceOfMarketOutcome(_market, _outcome, _owner);
        uint256 _availableCash = cashAvailableForTransferFrom(_owner, address(fillOrder));
        uint256 _attoSharesPurchasable = _availableCash.div(marketGetter.getNumTicks(_market).sub(_price));
        ...
        returns (uint256)
    {
        ...
        require(address(this).balance >= _protocolFee.mul(_maxTrades), "Not enough ETH provided to pay protocol fee");
        ...
        for (uint256 i = 0; i < _orders.length && _fillAmountRemaining != 0; i++) {
            ...
            uint256 _amountTraded = doTrade(_order, totalFillResults.takerAssetFilledAmount, _tradeGroupId, msg.sender);
    ...
    function cancelOrders(IExchange.Order[] memory _orders, bytes[] memory _signatures, uint256 _maxProtocolFeeDai) public returns (bool) {
        ...
        uint256 _protocolFee = exchange.protocolFeeMultiplier().mul(tx.gasprice);
        require(address(this).balance >= _protocolFee.mul(_orders.length), "Not enough ETH provided to pay protocol fee");
        transferFromAllowed = true;
        for (uint256 i = 0; i < _orders.length; i++) {
            ...
            IUniverse _universe = marketGetter.getUniverse(_orderData.marketAddress);
    ...
    function doTrade(IExchange.Order memory _order, uint256 _amount, bytes32 _tradeGroupId, address _taker) private returns (uint256 _amountFilled) {
        ...
        (uint256 _amountRemaining, uint256 _fees) = fillOrder.fillZeroXOrder(_augurOrderData.marketAddress, _augurOrderData.outcome, _augurOrderData.price, Order.Types(_augurOrderData.orderType), _order.makerAddress, _amount, _tradeGroupId, _taker);
    ...
    function logOrderFilled(IExchange.Order memory _order, AugurOrderData memory _augurOrderData, address _taker, bytes32 _tradeGroupId, uint256 _amountFilled, uint256 _fees) private {
        ...
        _uint256Data[8] = 0;
        _uint256Data[9] = 0;
        augurTrading.logZeroXOrderFilled(marketGetter.getUniverse(_augurOrderData.marketAddress), address(_augurOrderData.marketAddress), _orderHash, _tradeGroupId, uint8(_orderType), _addressData, _uint256Data);
    }
    ...
    function createZeroXOrderFor(address _maker, uint8 _type, uint256 _attoshares, uint256 _price, address _market, uint8 _outcome, uint256 _expirationTimeSeconds, uint256 _salt) public view returns (IExchange.Order memory _zeroXOrder, bytes32 _orderHash) {
        bytes memory _assetData = encodeAssetData(_market, _price, _outcome, _type);
