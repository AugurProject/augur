...
export class TestFixture {
    ...
    async createReasonableMarket(universe: Universe, outcomes: string[]): Promise<Market> {
        ...
        return this.createMarket(universe, outcomes, endTime, fee, affiliateFeeDivisor, this.account);
    }

    async placeOrder(market: string, type: BigNumber, numShares: BigNumber, price: BigNumber, outcome: BigNumber, betterOrderID: string, worseOrderID: string, tradeGroupID: string): Promise<void> {
    ...
    async contribute(market: Market, payoutNumerators: BigNumber[], amount: BigNumber): Promise<void> {
        await market.contribute(payoutNumerators, amount, '');
        return;
    }

    async derivePayoutDistributionHash(market: Market, payoutNumerators: BigNumber[]): Promise<string> {
    ...
    async migrateOutByPayout(reputationToken: TestNetReputationToken, payoutNumerators: BigNumber[], attotokens: BigNumber) {
    ...
    async getReportingParticipant(reportingParticipantAddress: string): Promise<DisputeCrowdsourcer> {
    ...
    async getWinningReportingParticipant(market: Market): Promise<DisputeCrowdsourcer> {
    ...
    async doInitialReport(market: Market, payoutNumerators: BigNumber[]): Promise<void> {
    ...
    async getLegacyRepAllowance(owner: string, spender: string): Promise<BigNumber> {
        const legacyRepContract = await this.contractDeployer.getContractAddress('LegacyReputationToken');
        const legacyRep = new LegacyReputationToken(this.dependencies, legacyRepContract);
    ...
    async getChildUniverseReputationToken(parentPayoutDistributionHash: string) {
        ...
        return new TestNetReputationToken(this.dependencies, repContractAddress);
    }
