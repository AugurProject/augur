...
export class ReportingUtils {
    ...
    public async proceedToInitialReporting(
        fixture: TestFixture,
        market: Market
    ) {
        const designatedReportingEndTime = await market.getDesignatedReportingEndTime_();
        await fixture.setTimestamp(
            designatedReportingEndTime.plus(new BigNumber(1))
        );
    }

    // TODO: Add `contributor` param, like `proceedToNextRound` function in Python tests
    public async proceedToNextRound(
        fixture: TestFixture,
        market: Market,
        doGenerateFees: boolean = false,
        moveTimeForward: boolean = true,
        randomPayoutNumerators: boolean = false
    ) {
        ...
        if (currentTimestamp.lt(marketEndTime)) {
            ...
            await fixture.setTimestamp(
                marketDesignatedReportingEndTime.plus(new BigNumber(1))
            );
        }
        ...
        let payoutNumerators = new Array(numberOfOutcomes.toNumber()).fill(
            new BigNumber(0)
        );
        ...
        if (disputeWindowAddress === ZERO_ADDRESS) {
            ...
            expect((await market.getDisputeWindow_()) === ZERO_ADDRESS).to.be
                .false;
            ...
            let numOwnedShares = await fixture.getNumSharesInMarket(
                market,
                outcome
            );
            ...
            console.log(
                'numOwnedShares after buying complete set',
                numOwnedShares.toString()
            );

            // Sell Complete Sets
            let numOwnedSharesBefore = await fixture.getNumSharesInMarket(
                market,
                outcome
            );
            console.log(
                'numOwnedShares before selling complete set',
                numOwnedSharesBefore.toString()
            );
            await fixture.sellCompleteSets(market, numShares);
            numOwnedSharesBefore = await fixture.getNumSharesInMarket(
                market,
                outcome
            );
            console.log(
                'numOwnedShares after selling complete set',
                numOwnedSharesBefore.toString()
            );
        } else {
            ...
            await fixture.setTimestamp(
                disputeWindowStartTime.plus(new BigNumber(1))
            );
            ...
            const winningReport = await fixture.getWinningReportingParticipant(
                market
            );
            winningPayoutHash = await winningReport.getPayoutDistributionHash_();

            let chosenPayoutNumerators = new Array(
                numberOfOutcomes.toNumber()
            ...
            if (randomPayoutNumerators) {
                ...
                chosenPayoutNumerators[1] = new BigNumber(
                    Math.floor(
                        Math.random() * Math.floor(numTicks.toNumber() + 1)
                    )
                );
                chosenPayoutNumerators[2] = numTicks.minus(
                    chosenPayoutNumerators[1]
                );
            } else {
                const firstReportWinning =
                    (await market.derivePayoutDistributionHash_(
                        payoutNumerators
                    )) === winningPayoutHash;
            ...
            console.log(
                'Reporting with payout numerators: ',
                chosenPayoutNumerators
            );
            const chosenPayoutHash = await market.derivePayoutDistributionHash_(
                chosenPayoutNumerators
            );
            const participantStake = await market.getParticipantStake_();
            const stakeInOutcome = await market.getStakeInOutcome_(
                chosenPayoutHash
            );
            const amount = participantStake
                .multipliedBy(new BigNumber(2))
                .minus(stakeInOutcome.multipliedBy(new BigNumber(3)));
            ...
            expect(
                forkingMarket !== ZERO_ADDRESS ||
                    marketDisputeWindow !== disputeWindowAddress
            ).to.be.true;
        }
    ...
    public async proceedToFork(
        ...
        let ethBalance = await fixture.getEthBalance();
        console.log(
            'ethBalance before calling for and redeem',
            ethBalance.toString()
        );

        const numParticipants = await market.getNumParticipants_();
        for (let i = 0; i < numParticipants.toNumber(); i++) {
            const reportingParticipantAddress = await market.participants_(
                new BigNumber(i)
            );
            const reportingParticipant = await fixture.getReportingParticipant(
                reportingParticipantAddress
            );
            console.log(`Creating universe for participant: ${i}`);
            await universe.createChildUniverse(
        ...
        console.log(
            'ethBalance after calling fork and redeem',
            ethBalance.toString()
        );
