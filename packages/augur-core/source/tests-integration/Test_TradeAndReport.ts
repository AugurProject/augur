...
describe('TradeAndReport', () => {
    ...
    it('#tradeAndReport', async () => {
        ...
        for (let i = 0; i < 22; i++) {
            await fixture.faucetRep(new BigNumber(499999).times(1e18));
        }
        ...
        const market = await fixture.createReasonableMarket(fixture.universe!, [
            stringTo32ByteHex(' '),
            stringTo32ByteHex(' '),
        ]);

        const migrationMarketEndTime = new BigNumber(
            Math.round(new Date().getTime() / 1000) + 365 * 24 * 60 * 60
        );
        const ZERO = new BigNumber(0);
        const migrationMarket = await fixture.createMarket(
            fixture.universe!,
            [stringTo32ByteHex(' '), stringTo32ByteHex(' ')],
            migrationMarketEndTime,
            ZERO,
            ZERO,
            fixture.account
        );
        ...
        const orderID = await fixture.getBestOrderId(
            type,
            market.address,
            outcome
        );
        ...
        const numOwnedShares = await fixture.getNumSharesInMarket(
            market,
            outcome
        );
        ...
        await reportingUtils.proceedToFork(
            fixture,
            market,
            await fixture.getUniverse(market)
        );
        ...
        const payoutDistributionHash = await fixture.derivePayoutDistributionHash(
            market,
            [new BigNumber(0), numTicks, new BigNumber(0)]
        );
        const childUniverseReputationToken = await fixture.getChildUniverseReputationToken(
            payoutDistributionHash
        );
        const initialRepTotalMigrated = await childUniverseReputationToken.getTotalMigrated_();
        expect(
            initialRepTotalMigrated ===
                new BigNumber('366666666666666667016192')
        ); // TODO: calculate this value instead of hard-coding it
        const repAmountToMigrate = new BigNumber(18000000).multipliedBy(
            new BigNumber(10).pow(18)
        );
        await fixture.migrateOutByPayout(
            reputationToken,
            [new BigNumber(0), numTicks, new BigNumber(0)],
            repAmountToMigrate
        );
        ...
        expect(
            finalRepTotalMigrated.minus(initialRepTotalMigrated).toString()
        ).to.equal(repAmountToMigrate.toString());
