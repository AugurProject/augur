...
contract Augur is IAugur {
    ...
    event MarketVolumeChanged(address indexed universe, address indexed market, uint256 volume, uint256 shareVolume);
    ...
    function logMarketVolumeChanged(IUniverse _universe, address _market, uint256 _volume, uint256 _shareVolume) public returns (bool) {
        require(msg.sender == registry["FillOrder"]);
        emit MarketVolumeChanged(address(_universe), _market, _volume, _shareVolume);
        return true;
    }
