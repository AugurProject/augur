...
const PRODUCTION_LEGACY_REP_CONTRACT_ADDRESS = Address.fromStringLiteral("0x1985365e9f78359a9B6AD760e32412f4a445E862");
const PRODUCTION_CASH_CONTRACT_ADDRESS = Address.fromStringLiteral("0x0000000000000000000000000000000000000000"); // TODO when MC CASH is released
...
export class DeployerConfiguration {
    ...
    public readonly augurAddress: Address|undefined;
    ...
    public readonly cashAddress: Address;

    public constructor(contractInputRoot: string, artifactOutputRoot: string, augurAddress: Address|undefined, createGenesisUniverse: boolean=true, isProduction: boolean=false, useNormalTime: boolean=true, legacyRepAddress: Address=PRODUCTION_LEGACY_REP_CONTRACT_ADDRESS, cashAddress: Address=PRODUCTION_CASH_CONTRACT_ADDRESS) {
    ...
    public static create(contractInputRoot:string=path.join(__dirname, '../../output/contracts'), artifactOutputRoot: string=ARTIFACT_OUTPUT_ROOT, isProduction: boolean=false, legacyRepAddress: Address=PRODUCTION_LEGACY_REP_CONTRACT_ADDRESS, cashAddress: Address=PRODUCTION_CASH_CONTRACT_ADDRESS): DeployerConfiguration {
        const augurAddress = Address.fromStringLiteral(process.env.AUGUR_ADDRESS || "");
    ...
    public static createWithControlledTime(legacyRepAddress: Address=PRODUCTION_LEGACY_REP_CONTRACT_ADDRESS, isProduction: boolean=false, artifactOutputRoot: string=ARTIFACT_OUTPUT_ROOT, cashAddress: Address=PRODUCTION_CASH_CONTRACT_ADDRESS): DeployerConfiguration {
