...
async function asyncFilter(
    arr: Array<any>,
    filterFunction: (item: any) => Promise<boolean>
): Promise<Array<any>> {
    const fail = Symbol();
    return (await Promise.all(
        arr.map(async item => ((await filterFunction(item)) ? item : fail))
    )).filter(i => i !== fail);
}

export async function recursiveReadDir(
    dir: string,
    ignore: (file: string, stats: fs.Stats) => boolean,
    allFiles: string[] = []
): Promise<string[]> {
    ...
    await Promise.all(
        files.map(
            async f =>
                (await statP(f)).isDirectory() &&
                recursiveReadDir(f, ignore, allFiles)
        )
    );
