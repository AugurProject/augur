...
export class ContractCompiler {
    ...
    private async getCommandOutputFromInput(
        ...
        return new Promise<string>((resolve, reject) => {
            if (
                ...
                throw Error(
                    'ChildProcess fields stdin, stdout, and stderr must not be null.'
                );
            }
    ...
    async compileContracts(): Promise<CompilerOutput> {
        ...
        if (compilerOutput.errors) {
            ...
            for (const error of compilerOutput.errors) {
                ...
                if (error.message.includes('Unused function parameter'))
                    continue;
                if (
                    error.message.includes(
        ...
        await fs.writeFile(
            this.configuration.fullContractOutputPath,
            JSON.stringify(
                {
                    compilerVersion,
                    ...compilerOutput,
    ...
    async generateFlattenedSolidity(filePath: string): Promise<string> {
        ...
        const formattedCommand = format(
            this.flattenerCommand,
            this.configuration.contractSourceRoot,
            relativeFilePath
        );
    ...
    async generateCompilerInput(): Promise<CompilerInput> {
        const ignoreFile = (file: string, stats: fs.Stats): boolean => {
            const allowedFilenames = INTERNAL_CONTRACTS.concat(
                EXTERNAL_CONTRACTS
            ).concat(TEST_CONTRACTS);
        ...
        if (this.configuration.useFlattener) {
            filesPromises = filePaths.map(async filePath =>
                this.generateFlattenedSolidity(filePath)
            );
        } else {
            filesPromises = filePaths.map(async filePath =>
    ...
    private filterCompilerOutput(
        ...
        for (const relativeFilePath in compilerOutput.contracts) {
            for (const contractName in compilerOutput.contracts[
                relativeFilePath
            ]) {
                ...
                if (this.configuration.enableSdb) {
                    ...
                    if (
                        ...
                        deployedBytecode.object === undefined ||
                        deployedBytecode.sourceMap === undefined
                    )
                        continue;
                    ...
                    result.contracts[relativeFilePath][
                        contractName
                    ].evm.bytecode.sourceMap = bytecode.sourceMap;
                    result.contracts[relativeFilePath][
                        contractName
                    ...
                    result.contracts[relativeFilePath][
                        contractName
                    ].evm.methodIdentifiers = JSON.parse(
                        JSON.stringify(methodIdentifiers)
                    );
                }
            }
        }
        ...
        if (
            this.configuration.enableSdb &&
            compilerOutput.sources !== undefined
        ) {
            result.sources = {};
            for (const relativeFilePath in compilerOutput.sources) {
                if (relativeFilePath in result.contracts) {
                    ...
                    result.sources[relativeFilePath] = {
                        ...
                        ast: JSON.parse(
                            JSON.stringify(
                                compilerOutput.sources[relativeFilePath]
                                    .legacyAST
                            )
                        ),
                        legacyAST: JSON.parse(
                            JSON.stringify(
                                compilerOutput.sources[relativeFilePath]
                                    .legacyAST
                            )
