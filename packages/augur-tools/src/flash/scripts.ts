...
import Routes from '@augurproject/ui/src/modules/sitemap/routes';
import fs from 'fs';
...
export function addScripts(flash: FlashSession) {
  ...
  flash.addScript({
    ...
    options: [{
      name: 'stable-loader-ipfs-hash',
      abbr: 'h',
      description: 'The IPFS hash from the stable loader file',
    }],
    async call(this: FlashSession, args: FlashArguments): Promise<void> {
      const address =
        process.env.NODE_ENV === 'development'
          ? 'https://v2.augur.net/#!'
          : `https://cloudflare-eth.com/${args.stableLoaderIpfsHash ||
          'QmVfCcpugkz2WRCWJ75jnDSKAGXAQEgV8XEd839An6aXLE'}/#!`;

      new Sitemap(Routes).build(address).save('@augurproject/ui/src/sitemap.xml');
