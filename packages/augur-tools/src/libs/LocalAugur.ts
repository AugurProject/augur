...
export async function makeTestAugur(seed: Seed, accounts: Account[]): Promise<Augur> {
  ...
  const dependencies = await makeGSNDependencies(provider, signer, seed.addresses.AugurWalletRegistry, seed.addresses.EthExchange, seed.addresses.WETH9, seed.addresses.Cash, accounts[0].publicKey);
