...
function tickedPriceIncrease(priceIncrease, tickSize, numTicks) {
  return Math.abs(
    priceIncrease < tickSize
      ? tickSize
      : Math.ceil((priceIncrease - (priceIncrease % tickSize)) * numTicks) /
        numTicks
  );
}
...
getPrivateKey(null, function(err, auth) {
  ...
  augur.connect(
    ...
    function(err) {
      ...
      approveAugurEternalApprovalValue(augur, auth.address, auth, function(
        err
      ) {
        if (err) {
        ...
        augur.markets.getMarketsInfo({ marketIds: [marketId] }, function(
          ...
          if (!marketsInfo || marketsInfo.length === 0)
            return console.error("Market not found");
          ...
          console.log(
            chalk.yellow.dim("outcomes:"),
            chalk.yellow(marketInfo.numOutcomes)
          );

          // Create bids/asks on each outcome around a midpoint
          for (
            var outcomeI = 0;
            outcomeI < marketInfo.numOutcomes;
            outcomeI++
          ) {
            console.log(chalk.yellow.dim("outcome:"), chalk.yellow(outcomeI));
            console.log(
              chalk.yellow.dim("max price:"),
              chalk.yellow(marketInfo.maxPrice)
            );
            console.log(
              chalk.yellow.dim("min price:"),
              chalk.yellow(marketInfo.minPrice)
            );
            ...
            var midPoint =
              Math.random() * (marketInfo.maxPrice - marketInfo.minPrice) +
              marketInfo.minPrice;
            midPoint =
              Math.ceil((midPoint - (midPoint % tickSize)) * numTicks) /
              numTicks;

            // Create Bids
            var bidPriceIncrease =
              (midPoint - marketInfo.minPrice) / numberOfOrders;
            bidPriceIncrease = tickedPriceIncrease(
              bidPriceIncrease,
              tickSize,
              numTicks
            );
            ...
            var bidPrice =
              marketInfo.minPrice === 0
                ? marketInfo.minPrice + bidPriceIncrease
                : marketInfo.minPrice;
            ...
            while (bidPrice < midPoint) {
              ...
              console.log(
                chalk.yellow.dim("bid:"),
                chalk.yellow(JSON.stringify(bid))
              );
              createOrder(
                augur,
                marketId,
                ...
                function(err, res) {
                  ...
                  console.log(
                    chalk.green.dim("Order Created"),
                    chalk.green(JSON.stringify(res))
                  );
                }
              );
            ...
            // Create Asks
            var askPriceIncrease =
              (marketInfo.maxPrice - midPoint) / numberOfOrders;
            askPriceIncrease = tickedPriceIncrease(
              askPriceIncrease,
              tickSize,
              numTicks
            );
            ...
            while (askPrice < marketInfo.maxPrice) {
              ...
              console.log(
                chalk.yellow.dim("ask:"),
                chalk.yellow(JSON.stringify(ask))
              );
              createOrder(
                augur,
                marketId,
