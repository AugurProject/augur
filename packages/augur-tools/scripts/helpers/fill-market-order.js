...
getPrivateKey(null, function(err, auth) {
  ...
  augur.connect(
    ...
    function(err) {
      ...
      approveAugurEternalApprovalValue(augur, fillerAddress, auth, function(
        err
      ) {
        if (err) {
          console.error(err);
        ...
        augur.markets.getMarketsInfo({ marketIds: [marketId] }, function(
          ...
          console.log(
            chalk.yellow.dim("outcomeToFill"),
            chalk.yellow(outcomeToFill)
          );
          console.log(
            chalk.yellow.dim("filler address"),
            chalk.yellow(fillerAddress)
          );
          augur.trading.getOrders(
            {
              marketId: marketId,
              outcome: outcomeToFill,
              orderType: orderType
            },
            function(err, orderBook) {
              ...
              if (
                !orderBook[marketId] ||
                !orderBook[marketId][outcomeToFill] ||
                !orderBook[marketId][outcomeToFill][orderType]
              ) {
              ...
              console.log(
                chalk.red.bold("num orders: "),
                chalk.red(Object.keys(orders).length)
              );
              Object.keys(orders).forEach(function(orderId) {
                var order = orders[orderId];
                if (
                  order.orderState !== "CANCELED" &&
                  orders[orderId].owner !== fillerAddress
                ) {
                  if (order == null) {
