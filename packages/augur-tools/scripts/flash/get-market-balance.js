...
function help() {
  console.log(
    chalk.red("Get share tokens on market outcomes for user account")
  );
...
function showWinningBalance(augur, marketId, address, callback) {
  ...
  augur.augurNode.submitRequest("getWinningBalance", winningPayload, function(
    err,
    value
  ) {
    if (err) {
...
function showCashBalance(augur, address, label, callback) {
  augur.api.Cash.balanceOf({ _owner: address }, function(err, cashBalance) {
    ...
    console.log(
      chalk.green.dim("Total Cash balance:"),
      chalk.green(totalCashBalance)
    );
    callback(null);
  });
}
...
function showEthBalance(augur, address, label, callback) {
  augur.rpc.eth.getBalance([address, "latest"], function(err, attoEthBalance) {
    ...
    var totalCashBalance = speedomatic.unfix(bnCashBalance, "string");
    console.log(
      chalk.green.dim("Total ETH balance:"),
      chalk.green(totalCashBalance)
    );
    callback(null);
  });
...
function getMarketBalance(augur, args, auth, callback) {
  ...
  augur.markets.getMarketsInfo({ marketIds: [marketId] }, function(
    ...
    var showMarketWinningBalance =
      market.reportingState === augur.constants.REPORTING_STATE.FINALIZED ||
      market.reportingState ===
        augur.constants.REPORTING_STATE.AWAITING_FINALIZATION;
    ...
    console.log(
      chalk.green.dim("Mailbox owner:"),
      chalk.green(market.marketCreatorMailboxOwner)
    );
    async.series(
      {
        ...
        showWinningBalance: function(next) {
          if (address && showMarketWinningBalance) {
            console.log(
              chalk.yellow.dim("Market State"),
              chalk.yellow(market.reportingState)
            );
                ...
                augur.api.Market.getShareToken(
                  ...
                  function(err, shareToken) {
                    ...
                    augur.api.ShareToken.balanceOf(shareTokenPayload, function(
                      ...
                      var adjusted = new BigNumber(
                        speedomatic.unfix(balance, "string")
                      ).times(new BigNumber(numTicks));
                      console.log(
                        chalk.yellow.dim("Market Outcome: "),
                        chalk.yellow(outcomeId)
                      );
                      console.log(
                        "attoShare: " + chalk.green(balance),
                        "Shares: " +
                          chalk.green(speedomatic.unfix(balance, "string")),
                        "Adjusted Shares: " + chalk.green(adjusted)
                      );
                      nextOutcome(null);
                    });
                  }
