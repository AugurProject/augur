...
ADDRESSES="../artifacts/src/addresses.json"
BLOCKS="../artifacts/src/upload-block-numbers.json"

echo "processing $IMAGE $ADDRESSES"
docker run --rm --entrypoint cat $IMAGE /augur/packages/artifacts/addresses.json > $TEMP1
node scripts/merge-json-files -p $ADDRESSES -s $TEMP1 -o $ADD_TEMP2
ADD_RESULT_CODE=$?

echo "processing $IMAGE $BLOCKS"
docker run --rm --entrypoint cat $IMAGE /augur/packages/artifacts/upload-block-numbers.json > $TEMP1
