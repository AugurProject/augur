...
export function byteArrayToUtf8String(
  byteArray: Buffer | string | BigNumber | Number
) {
  let el, byteString;
  ...
  if (byteArray.constructor === String) {
    ...
    byteArray.constructor === Number ||
    BigNumber.isBigNumber(byteArray)
  ) {
  ...
  try {
    ...
    console.error(
      '[@augurproject/utils] byteArrayToUtf8String:',
      JSON.stringify(byteString, null, 2)
    );
    throw ex;
  }
