...
describe('Checkpoint', () => {
  ...
  describe('timestamp between blocks', () => {
    test('should return blocks directly above and below', async () => {
      ...
      ).resolves.toEqual([blocks[2], blocks[3]]);
    });
  });
  ...
  describe('timestamp exactly as lower block in range', () => {
    test('should return the matching block and the one after', async () => {
      await expect(
        checkpoints.calculateBoundary(blocks[2].timestamp, blocks[0], blocks[4])
      ).resolves.toEqual([blocks[2], blocks[3]]);
    });
  });
  ...
  describe('timestamp exactly the same as upper block in range', () => {
    test('should throw an exception', async () => {
      await expect(
        checkpoints.calculateBoundary(blocks[3].timestamp, blocks[2], blocks[3])
      ).rejects.toEqual(new Error('timestamp outside of provided block range'));
    });
  });
  ...
  describe('timestamp above blocks provided', () => {
    test('should throw an exception', async () => {
      await expect(
        checkpoints.calculateBoundary(blocks[4].timestamp, blocks[2], blocks[3])
      ).rejects.toEqual(new Error('timestamp outside of provided block range'));
    });
  });
  ...
  describe('timestamp below blocks provided', () => {
    test('should throw an exception', async () => {
      await expect(
        checkpoints.calculateBoundary(blocks[0].timestamp, blocks[2], blocks[3])
      ).rejects.toEqual(new Error('timestamp outside of provided block range'));
    });
  });
});
