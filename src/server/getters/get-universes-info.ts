...
export function getUniversesInfo(db: Knex, augur: Augur, universeId: Address, account: Address, callback: (err: Error|null, result?: any) => void): void {
  if (universeId == null || account == null) return callback(new Error("Must provide both universe and account"));

  const query = db.select("parentUniverse").from("universes").where("universe", universeId).first()

  query.asCallback((err: Error|null, parentUniverseRow: ParentUniverseRow) => {
    if (err) return callback(err);

    let query = db.select(["universes.universe", "universes.parentUniverse", "balances.balance", "token_supply.supply", db.raw("count(markets.marketId) as numMarkets")]).from("universes")
      .where("universes.parentUniverse", universeId) // Children
      .orWhere("universes.parentUniverse", parentUniverseRow.parentUniverse) // Siblings
      .orWhere("universes.universe", universeId) // Universe
