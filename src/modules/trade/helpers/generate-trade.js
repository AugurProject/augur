...
export const generateTrade = memoize((market, outcome, outcomeTradeInProgress, orderBooks) => {
  ...
  const totalFee = WrappedBigNumber((outcomeTradeInProgress && outcomeTradeInProgress.totalFee) || '0', 10)
  ...
  const totalCost = WrappedBigNumber((outcomeTradeInProgress && outcomeTradeInProgress.totalCost) || '0', 10)
  ...
  if (tradeOrders && tradeOrders.length) {
    tradeSummary = tradeOrders.reduce((p, tradeOrder) => {
      ...
      if (tradeOrder.data && tradeOrder.data.gasFees && tradeOrder.data.gasFees.value) {
        p.totalGas = p.totalGas.plus(WrappedBigNumber(tradeOrder.data.gasFees.value, 10))
      }
...
export const generateTradeOrders = memoize((market, outcome, outcomeTradeInProgress) => {
  ...
  return tradeActions.map((tradeAction) => {
    const numShares = WrappedBigNumber(tradeAction.shares, 10)
    const costEth = WrappedBigNumber(tradeAction.costEth, 10).abs()
    const avgPrice = WrappedBigNumber(costEth, 10).dividedBy(WrappedBigNumber(numShares, 10))
