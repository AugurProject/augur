export const buildSearchString = (keywords, tags) => {
  ...
  let keywordSearch = keywords && keywords.length > MIN_KEYWORDS_LENGTH ? keywords : undefined
  if (keywordSearch && (!keywordSearch.endsWith(' ') && !keywordSearch.endsWith('"'))) keywordSearch += '*'
  const terms = [];
