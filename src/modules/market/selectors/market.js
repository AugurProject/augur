    ...
    assembledMarketsCache[marketId] = memoize((
      ...
      market.outcomes = Object.keys(marketOutcomesData || {}).map((outcomeId) => {
        ...
        if (market.isScalar) {
          ...
          if (outcome.lastPrice.value) {
            ...
            const midPoint = (WrappedBigNumber(market.minPrice, 10).plus(WrappedBigNumber(market.maxPrice, 10))).dividedBy(2)
      ...
      if (marketData.consensus) {
        ...
        if (market.consensus.proportionCorrect) {
          market.consensus.percentCorrect = formatPercent(WrappedBigNumber(market.consensus.proportionCorrect, 10).times(100))
        }
      } else {
