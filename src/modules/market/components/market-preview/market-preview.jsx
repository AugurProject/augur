...
const MarketPreview = p => (
  <article className={classNames(CommonStyles.MarketCommon__container, { [`${CommonStyles['single-card']}`]: p.cardStyle === 'single-card' })} id={'id-' + p.id} data-testid={p.testid + '-' + p.id} orderid={p.testid + '-' + p.orderid + '-' + p.id}>
    <MarketBasics {...p} />
