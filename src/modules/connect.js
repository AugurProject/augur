...
module.exports = {
  ...
  //     if (!tx.send) {
  //       if (params && params[0] != null && isObject(params[0])) {
  ...
  //       }
  //       if (!isFunction(cb)) return self[tx.parser](rpcInterface.callContractFunction(tx));
  //       return rpcInterface.callContractFunction(tx, cb, self[tx.parser]);
  //     }
  ...
  //     }
  //     if (!tx.parser) {
  //       return rpcInterface.transact(tx, onSent, onSuccess, onFailed);
  //     }
  //     return rpcInterface.transact(tx, onSent, compose(self[tx.parser], onSuccess), onFailed);
  //   };
  // },
  ...
  connect: function (connectOptions, callback) {
    ...
    if (!isFunction(callback)) {
      ...
      // this.rpc = vitals.rpc;
      this.filters = filters(createRpcInterface(vitals.rpc).getBlockStream);
      return vitals;
    }
    connector.connect(options, function (err, vitals) {
      ...
      // self.rpc = vitals.rpc;
      self.filters = filters(rpcInterface.createRpcInterface(vitals.rpc).getBlockStream);
      callback(vitals);
    });
  }
};
