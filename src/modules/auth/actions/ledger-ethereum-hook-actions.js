...
export function onConnectLedgerRequest() {
  return (dispatch, getState) => {
    dispatch(updateAuthStatus(LEDGER_STATUS, LEDGER_STATES.CONNECT_LEDGER));
  };
}

export function onOpenEthereumAppRequest() {
  return (dispatch, getState) => {
    dispatch(updateAuthStatus(LEDGER_STATUS, LEDGER_STATES.OPEN_APP));
  };
}

export function onSwitchLedgerModeRequest() {
  return (dispatch, getState) => {
    dispatch(updateAuthStatus(LEDGER_STATUS, LEDGER_STATES.SWITCH_MODE));
  };
}
...
export function onEnableContractSupportRequest() {
  return (dispatch, getState) => {
    dispatch(
      updateAuthStatus(LEDGER_STATUS, LEDGER_STATES.ENABLE_CONTRACT_SUPPORT)
    );
    dispatch(
      updateModal({
        type: MODAL_LEDGER,
