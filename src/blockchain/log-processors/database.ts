...
export function insertPayout(trx: Knex.Transaction, marketID: Address, payoutNumerators: Array<string|number|null>, invalid: boolean, tentativeWinning: boolean, callback: (err: Error|null, payoutID?: number) => void): void {
  ...
  trx.select("payoutID").from("payouts").where(payoutRow).first().asCallback( (err: Error|null, payoutID?: number|null): void => {
    if (err) return callback(err);
    if (payoutID != null) {
      return callback(null, payoutID);
    } else {
      trx.insert(payoutRow).returning("payoutID").into("payouts").asCallback((err: Error|null, payoutIDRow?: Array<number>): void => {
        if (err) callback(err);
