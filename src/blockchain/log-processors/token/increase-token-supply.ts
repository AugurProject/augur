...
export function increaseTokenSupply(db: Knex, augur: Augur, token: Address, amount: number, callback: ErrorCallback): void {
  db.first("supply").from("token_supply").where({ token }).asCallback((err: Error|null, oldSupply?: SupplyResult): void => {
    if (err) return callback(err);
    if (oldSupply == null) {
      db.insert({ token, supply: amount }).into("token_supply").asCallback(callback);
    } else {
