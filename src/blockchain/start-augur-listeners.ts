...
export function startAugurListeners(db: Knex, augur: Augur, highestBlockNumber: number, databaseDir: string, isWarpSync: boolean, errorCallback: ErrorCallback): BlockAndLogsQueue {
  const blockAndLogsQueue = new BlockAndLogsQueue(async (direction: BlockDirection, block: BlockDetail, logs: Array<FormattedEventLog>) => {
    return processBlockAndLogs(db, augur, direction, block, false, logs, databaseDir, isWarpSync).catch((err) => {
      errorCallback(err);
      throw(err);
    });
  });
