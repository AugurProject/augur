...
export function onNewBlock(db: Knex, trx: Knex.Transaction, augur: Augur, blockNumber: string) {
  augur.rpc.eth.getBlockByNumber([blockNumber, false], (block: any): void => {
    ...
    trx.raw(`INSERT OR REPLACE INTO blocks (block_number, block_timestamp) VALUES (?, ?)`, dataToInsertOrReplace).asCallback(logError);
  });
}
