...
import { ErrorCallback, FormattedLog } from "../types";
...
export function downloadAugurLogs(db: Knex, augur: Augur, fromBlock: number, toBlock: number, callback: ErrorCallback): void {
  ...
  augur.events.getAllAugurLogs({ fromBlock, toBlock }, (err?: string|object|null, allAugurLogs?: Array<FormattedLog>): void => {
    if (err) return callback(err instanceof Error ? err : new Error(JSON.stringify(err)));
    eachSeries(allAugurLogs!, (log: FormattedLog, nextLog: ErrorCallback) => {
