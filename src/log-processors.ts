import { SqlLiteDb, FormattedLog, LogProcessor, ErrorCallback } from "./types";

export const logProcessors: {[contractName: string]: {[eventName: string]: LogProcessor}} = {
  Augur: {
    MarketCreated: (db: SqlLiteDb, log: FormattedLog, callback: ErrorCallback): void => {
    ...
    TokensTransferred: (db: SqlLiteDb, log: FormattedLog, callback: ErrorCallback): void => {
