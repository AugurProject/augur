...
exports.up = async (knex: Knex): Promise<any> => {
  return knex.schema.dropTableIfExists("trades").then(() => {
    return knex.schema.raw(`CREATE TABLE trades (
      ...
      num_creator_shares integer NOT NULL CONSTRAINT nonnegative_num_creator_shares CHECK (num_creator_shares >= 0),
      num_creator_tokens integer NOT NULL CONSTRAINT nonnegative_num_creator_tokens CHECK (num_creator_tokens >= 0),
      num_filler_shares integer NOT NULL CONSTRAINT nonnegative_num_filler_shares CHECK (num_filler_shares >= 0),
