...
exports.up = async (knex: Knex): Promise<any> => {
  return knex.raw("DROP VIEW IF EXISTS all_participants").then((): PromiseLike<any> => {
    return knex.raw(`CREATE VIEW all_participants AS
      SELECT markets.universe, markets.marketId, 'initial_report' as type, initialReporter as participantAddress, initial_reports.reporter, initial_reports.blockNumber, amountStaked as size, CASE  WHEN redeemed = 0 THEN amountStaked ELSE 0 END as reporterBalance, contractBalances.token as reputationToken, contractBalances.balance as reputationTokenBalance, initial_reports.payoutId, 1 as completed, reportingState, markets.forking, disavowed from initial_reports
