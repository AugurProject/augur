...
exports.up = async (knex: Knex): Promise<any> => {
  return knex.schema.dropTableIfExists("markets").then((): PromiseLike<any> => {
    return knex.schema.raw(`CREATE TABLE markets (
      ...
      "marketCreatorFeesClaimed" varchar(255) DEFAULT "0" CONSTRAINT "nonnegativeMarketCreatorFeesClaimed" CHECK (ltrim("marketCreatorFeesClaimed", '-') = "marketCreatorFeesClaimed"),
      "marketCreatorFeesCollected" varchar(255) DEFAULT "0" CONSTRAINT "nonnegativeMarketCreatorFeesCollected" CHECK (ltrim("marketCreatorFeesCollected", '-') = "marketCreatorFeesCollected"),
