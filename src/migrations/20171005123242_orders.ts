...
exports.up = async (knex: Knex): Promise<any> => {
  return knex.schema.dropTableIfExists("orders").then(async (): Promise<any> => {
    return knex.schema.createTable("orders", (table: Knex.CreateTableBuilder): void => {
      // table.increments("orderID").primary().notNullable();
      ...
      table.specificType("orderState", "varchar(9) NOT NULL CONSTRAINT enumOrderStates CHECK (\"orderState\" = 'OPEN' OR \"orderState\" = 'CLOSED' OR \"orderState\" = 'CANCELLED')");
      table.specificType("creationBlockNumber", "integer NOT NULL CONSTRAINT positiveOrderCreationBlockNumber CHECK (\"creationBlockNumber\" > 0)");
