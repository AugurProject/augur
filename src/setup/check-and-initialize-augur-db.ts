...
export async function createDbAndConnect(augur: Augur, network: NetworkConfiguration, errorCallback: ErrorCallback | undefined, databaseDir?: string): Promise<Knex> {
  return new Promise<Knex>((resolve, reject) => {
    augur.connect({ ethereumNode: { http: network.http, ws: network.ws }, startBlockStreamOnConnect: false }, async (err) => {
      ...
      try {
        monitorEthereumNodeHealth(augur, errorCallback);
