...
geth $optargs --cache 2048 --networkid $network --datadir $symlink --rpc --rpcapi "eth,net,web3,admin,personal,miner,txpool" --ipcapi "admin,eth,debug,miner,net,txpool,personal,web3" --rpccorsdomain "http://local.augur.net,http://localhost:8080" --maxpeers $maxpeers --etherbase $address --unlock $address --password $passfile console
