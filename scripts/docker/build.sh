...
docker build --no-cache . --build-arg normal_time=false --build-arg network_id=102 -t $IMAGE_NAME:$TAG -t $IMAGE_NAME:latest
...
# Build and copy to merge with current address.json/block_numbers.json
IMAGE_NAME=augurproject/dev-pop-geth-45
docker build --no-cache . --build-arg normal_time=false --build-arg network_id=104 --build-arg period_time=45 -t $IMAGE_NAME:$TAG -t $IMAGE_NAME:latest
./scripts/copy-docker-files.sh $IMAGE_NAME:latest
